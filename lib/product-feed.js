...
const GOOGLE_PRODUCT_CATEGORY = {
  'Behr Color Matches': DEFAULT_GOOGLE_PRODUCT_CATEGORY,
  'Sherwin-Williams Color Matches': DEFAULT_GOOGLE_PRODUCT_CATEGORY,
  'Benjamin Moore Color Matches': DEFAULT_GOOGLE_PRODUCT_CATEGORY
};
...
function productUrl(product, code) {
  const base = `${SITE_ORIGIN}/palettes/${productSlug(product)}`;
...
function feedItem(product, code = DEFAULT_CURRENCY) {
  ...
  const item = {
    ...
    // The paint manufacturer a palette matches stays out of the feed's product
    // type: Pinterest's merchant review reads a manufacturer's name there as a
    // claim about who made the product.
    product_type: 'Paint Color Palettes',
...
// Pinterest's CSV data source.
//
// Pinterest accepts TSV, CSV and XML, but only the column-based formats are
// specified precisely enough to build against with confidence - their own
// validator reports errors as required_columns_missing, itemid_missing,
// title_missing, product_category_missing and price_missing, which are column
// errors. The RSS dialect below is kept for the existing data source; this is
// the format to point a new one at.
//
// Values follow the Google spelling ("in stock", "new") because that is the
// convention the column formats are documented against, and are lower case.
const PINTEREST_CSV_COLUMNS = [
  'id', 'title', 'description', 'link', 'image_link', 'price', 'availability',
  'condition', 'brand', 'mpn', 'product_type', 'google_product_category',
  'additional_image_link'
];

const PINTEREST_CSV_AVAILABILITY = {
  in_stock: 'in stock',
  out_of_stock: 'out of stock'
};

// RFC 4180: a field is quoted when it contains a comma, a quote or a line
// break, and quotes inside are doubled. Without this a description containing
// a comma silently shifts every later column, which is the classic way a CSV
// feed produces wrong data rather than an error.
function csvField(value) {
  const text = value === undefined || value === null ? '' : String(value);
  return /[",\r\n]/.test(text) ? `"${text.replace(/"/g, '""')}"` : text;
}

// Returns the reasons a product cannot be listed, so a skip can be logged with
// a cause instead of the item vanishing silently.
function pinterestRowProblems(item) {
  const problems = [];
  if (!item.id) problems.push('missing id');
  if (!item.title) problems.push('missing title');
  if (!item.description) problems.push('missing description');
  if (!item.link || !item.link.startsWith('https://')) problems.push('missing or non-https link');
  if (!item.image_link || !item.image_link.startsWith('https://')) problems.push('missing or non-https image_link');
  if (!/^\d+\.\d{2} [A-Z]{3}$/.test(item.price || '')) problems.push(`bad price "${item.price}"`);
  if (!PINTEREST_CSV_AVAILABILITY[item.availability]) problems.push(`bad availability "${item.availability}"`);
  return problems;
}

// Returns { csv, stats } - never throws. stats carries the counts and the
// per-product skip reasons for the caller to log.
function buildFeedCsv(products, options = {}) {
  const { currency = DEFAULT_CURRENCY } = options;
  const cfg = currencyConfig(currency);

  const rows = [PINTEREST_CSV_COLUMNS.join(',')];
  const skipped = [];
  let processed = 0;
  ...
  for (const product of products || []) {
    ...
    if (!isListable(product, cfg.code)) {
      skipped.push({ id: product && product.id, reason: `not listable in ${cfg.code}` });
      continue;
    }

    let item;
    try {
      item = feedItem(product, cfg.code);
    } catch (error) {
      skipped.push({ id: product && product.id, reason: `could not build item: ${error.message}` });
      continue;
    }

    const problems = pinterestRowProblems(item);
    if (problems.length) {
      skipped.push({ id: item.id, reason: problems.join('; ') });
      continue;
    }
    ...
    rows.push([
      ...
      item.image_link,
      item.price,
      PINTEREST_CSV_AVAILABILITY[item.availability],
      item.condition,
      item.brand,
      item.mpn || '',
      item.product_type || '',
      item.google_product_category || '',
      // Multiple images go in one field, comma separated - which is exactly
      // why the field has to be quoted.
  ...
  return {
    csv: rows.join('\r\n') + '\r\n',
    stats: { processed, valid: rows.length - 1, skipped }
  };
}
...
function buildFeedXml(products, options = {}) {
  ...
  const body = items.map(item => {
    ...
    if (pinterest) {
      // No g:tax block. It is a Google construct that Pinterest does not
      // document, and its <g:rate> is a bare number sitting among fields that
      // do carry a currency - which is what Pinterest reported as alert 180,
      // "fields with inconsistent currencies", on all 44 items. Dropping it
      // loses nothing: the rate was 0, so "no tax added" is what Pinterest
      // assumes anyway.
      lines.push('      <g:shipping>');
      lines.push(`          <g:country>${escapeXml(PINTEREST_SHIPPING_COUNTRY)}</g:country>`);
      // Two decimals, matching g:price. "0 USD" beside "16.00 USD" is the same
      // currency in two shapes, and a parser comparing price fields has no
      // reason to be given that.
      lines.push(`          <g:price>0.00 ${escapeXml(item.currency)}</g:price>`);
      lines.push('      </g:shipping>');
    }

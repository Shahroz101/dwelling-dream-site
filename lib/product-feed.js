...
// guide"; Decor is the closest honest fit for a home-colour lookbook, and it is
// where a buyer looking for this would browse. Change here if you disagree -
// it is one line, and it must be a verbatim path from Google's taxonomy:
// https://support.google.com/merchants/answer/6324436
const DEFAULT_GOOGLE_PRODUCT_CATEGORY = 'Home & Garden > Decor';
...
function feedItem(product) {
  ...
  const item = {
    ...
    image_link: images[0],
    additional_image_link: images.slice(1, 5),
...
// availability with a space ("in stock") rather than an underscore. Everything
// else is derived identically, so both catalogs always describe the same
// product with the same price, image and URL.
...
function buildFeedXml(products, options = {}) {
  const { dialect = 'google', generatedAt = new Date() } = options;
  const pinterest = dialect === 'pinterest';
  const items = products.filter(isListable).map(feedItem);

  const body = items.map(item => {
    const availability = pinterest
      ? (PINTEREST_AVAILABILITY[item.availability] || item.availability)
      : item.availability;
    const lines = [
    ...
    if (!pinterest) lines.push(`      <g:identifier_exists>${escapeXml(item.identifier_exists)}</g:identifier_exists>`);

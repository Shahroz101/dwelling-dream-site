...
// of a download link was the order page a buyer landed on after PayPal, so
...
function orderConfirmation({ order, orderUrl }) {
  ...
  const lines = items.map(item => {
    ...
    return `  - ${item.name}${qty}  ${money(item.price * (item.qty || 1))}`;
  });
  ...
  const rows = items.map(item => {
    ...
    return `<tr>
      <td style="padding:8px 0;border-bottom:1px solid #E5DFD4;color:#292825;">${escapeHtml(item.name)}${qty}</td>
      <td style="padding:8px 0;border-bottom:1px solid #E5DFD4;color:#292825;text-align:right;">${money(item.price * (item.qty || 1))}</td>
    </tr>`;
  }).join('');

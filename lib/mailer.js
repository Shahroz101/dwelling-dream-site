...
function saleNotification({ order, orderUrl, buyerEmail }) {
  ...
  const text = [
    ...
    '',
    `Buyer:     ${buyerEmail || 'NOT PROVIDED BY PAYPAL - no receipt could be sent'}`,
    `Reference: ${order.id}`,
  ...
  const buyerLine = buyerEmail
    ...
    : '<strong style="color:#8C3A2B;">not provided by PayPal &mdash; no receipt could be sent</strong>';
